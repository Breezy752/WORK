...
var xhr_default = typeof XMLHttpRequest !== "undefined" && function(config) {
	return new Promise(function dispatchXhrRequest(resolve, reject) {
		...
		request.ontimeout = function handleTimeout() {
			...
			reject(new AxiosError$1(timeoutErrorMessage, transitional.clarifyTimeoutError ? AxiosError$1.ETIMEDOUT : AxiosError$1.ECONNABORTED, _config, request));
			request = null;
		};
		...
		if (typeof _config.onDownloadProgress === "function") request.addEventListener("progress", progressEventReducer_default(_config.onDownloadProgress, true));
		if (typeof _config.onUploadProgress === "function" && request.upload) request.upload.addEventListener("progress", progressEventReducer_default(_config.onUploadProgress));
...
var getBodyLength = async (body) => {
	...
	if (utils_default.isSpecCompliantForm(body)) return (await new Request(body).arrayBuffer()).byteLength;
	if (utils_default.isArrayBufferView(body)) return body.byteLength;
	...
	fetch: isFetchSupported && (async (config) => {
		...
		try {
			if (onUploadProgress && supportsRequestStream && method !== "get" && method !== "head" && (requestContentLength = await resolveBodyLength(headers, data)) !== 0) {
				...
				if (_request.body) data = trackStream(_request.body, DEFAULT_CHUNK_SIZE, fetchProgressDecorator(requestContentLength, progressEventReducer_default(onUploadProgress)), null, encodeText);
			}
			if (!utils_default.isString(withCredentials)) withCredentials = withCredentials ? "cors" : "omit";
