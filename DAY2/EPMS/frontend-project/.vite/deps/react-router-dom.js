	...
	async function handleAction(request, location, submission, matches, isFogOfWar, opts) {
		...
		if (isFogOfWar) {
			...
			if (discoverResult.type === "aborted") return { shortCircuited: true };
				...
				let { error, notFoundMatches, route } = handleDiscoverRouteError(location.pathname, discoverResult);
				return {
					matches: notFoundMatches,
	...
	*/
	...
	function checkFogOfWar(matches, routesToUse, pathname) {
		if (patchRoutesOnMissImpl) {
			...
			else if (matches[matches.length - 1].route.path === "*") return {
				active: true,
				matches: matchRoutesImpl(routesToUse, pathname, basename, true)
			};
		}
	...
	async function discoverRoutes(matches, pathname, signal) {
		...
		while (true) {
			...
			if (newMatches) {
				...
				if (leafRoute.path && leafRoute.path.length > 0) if (leafRoute.path === "*") matchedSplat = true;
				else return {
					type: "success",
					matches: newMatches
				};
...
function getInternalRouterError(status, _temp5) {
	...
	if (status === 400) {
		...
		if (type === "route-discovery") errorMessage = "Unable to match URL \"" + pathname + "\" - the `children()` function for " + ("route `" + routeId + "` threw the following error:\n" + message);
